...
uint32_t FSocketThread::Run()
{
	...
	server_addr.sin_port = htons(7777); // 서버 포트 지정 (7777)
	inet_pton(AF_INET, IPAddress, &server_addr.sin_addr); // 서버 IP 설정

	// 소켓을 이용해 서버와 연결 시도
	ret = WSAConnect(ls_socket, reinterpret_cast<sockaddr*>(&server_addr), sizeof(server_addr), 0, 0, 0, 0);

	// 연결 실패 시 오류 코드에 따른 로그 출력
	if (ret != 0) {
	...
	// 로그인 및 회원가입 기능 비활성화 (디버그용)
	_gameInstance->DisableLoginSignupForDebug();
	
	// 스레드가 실행되는 동안 비동기 작업을 위한 대기 상태 유지
	while (IsRunning) {
		SleepEx(100, true);
	}
